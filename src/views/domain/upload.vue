        ...
        <div class="url_con" v-if="tableType">
            ...
            <el-table border :data="tableList" style="width: 100%" height="420px">
                ...
                <el-table-column label="视频格式（0:mp4 1:hls 2:flv）">
                    <template slot-scope="scope">
                        <span v-if="scope.row.urlType===false" style="color:red">{{scope.row.url_type}}</span>
                        <span v-else style="color:green">{{scope.row.url_type}}</span>
                    </template>
                </el-table-column>
                <el-table-column label="视频名称">
                    <template slot-scope="scope">
                        <span v-if="scope.row.urlName===false" style="color:red">{{scope.row.url_name}}</span>
                        <span v-else style="color:green">{{scope.row.url_name}}</span>
                    </template>
                </el-table-column>
                <el-table-column label="渠道ID">
                    <template slot-scope="scope">
                        <span v-if="scope.row.idType===false" style="color:red">{{scope.row.busercheck}}</span>
                        <span v-else style="color:green">{{scope.row.buser_id}}</span>
                    </template>
                </el-table-column>
                <el-table-column  label="标签">
                    <template slot-scope="scope">
                        <span v-if="scope.row.urllabel===false" style="color:red">{{scope.row.label}}</span>
                        <span v-else style="color:green">{{scope.row.label}}</span>
                    </template>
                </el-table-column>
                <el-table-column  label="终端ID">
                      <template slot-scope="scope">
                        <span v-if="scope.row.urllabel2===false" style="color:red">{{scope.row.label2}}</span>
            ...
            if (res.status == 0) {
                if (res.err_code == 0) {
                    ...
                    for (var i = 0; i < res.data.length; i++) {
                        ...
                        if (resType.test(res.data[i].url_type) === false) {
                        ...
                        if (resName.test(res.data[i].url_name) === false) {
