<template>
	<div class="simple">
		...
		<div class="con">
			...
			<el-upload class="upload-demo" ref="upload" :action="excel_url" :on-preview="handlePreview" :on-remove="handleRemove" :file-list="fileList" :on-success="handleAvatarSuccess" name="excel" :auto-upload="false">
				<el-button slot="trigger" size="small" class="MyctiveButton" type="primary" style="    position: absolute;top: 18px;left: 150px;padding: 12px 8px;">上传Excel文件</el-button>
				...
				<el-button style="position: absolute;left: 130px;bottom: 23px;width:85px;" size="small" type="success" :disabled="disable" @click="submitUploadOK">确定</el-button>
			</el-upload>
			<div class="url_con" v-if="tableType">
				<!-- 表格 -->
				<el-table border :data="tableList" style="width: 100%" height="400px" :cell-style="rowClass" :header-cell-style="headClass">
					<el-table-column label="加速内容名称" prop="url_name" width="280px">
						<template slot-scope="scope">
							<span style="color:red" v-if="scope.row.err_acc == false">{{ scope.row.url_name }}</span>
							<span style="color:green;" v-else>
								{{ scope.row.url_name }}
							</span>
						</template>
					</el-table-column>
					<el-table-column label="源站域名" prop="domain" width="280px">
						<template slot-scope="scope">
							<span style="color:red" v-if="scope.row.err_domain == false">{{ scope.row.domain }}</span>
							<span style="color:green;" v-else>
								{{ scope.row.domain }}
							</span>
						</template>
					...
					<el-table-column prop="host_url" label="回源路径">
						<template slot-scope="scope">
							<span style="color:red" v-if="scope.row.err_hosturl == false">{{ scope.row.host_url }}</span>
							<span style="color:green;" v-else>
								{{ scope.row.host_url }}
							</span>
						</template>
					...
					<el-table-column prop="url" label="播放路径">
						<template slot-scope="scope">
							<span style="color:red" v-if="scope.row.err_url == false">{{ scope.row.url }}</span>
							<span style="color:green;" v-else>
								{{ scope.row.url }}
							</span>
						</template>
					...
					<el-table-column prop="url_type" label="视频类型(0:mp4 1:hls 2:flv)" width="210px">
						<template slot-scope="scope">
							<span style="color:red" v-if="scope.row.err_url_type == false">{{ scope.row.url_type }}</span>
							<span style="color:green;" v-else>
								{{ scope.row.url_type }}
							</span>
						</template>
					</el-table-column>
					<el-table-column prop="buser_id" label="渠道ID" width="210px">
					
						<template slot-scope="scope">
						
							<span style="color:red" v-if="scope.row.err_buserid== false">{{ scope.row.buser_id }}</span>
							<span style="color:green;" v-else>
								{{ scope.row.buser_id }}
							</span>
						</template>
...
export default {
  ...
  mounted() {
    ...
    this.excel_url = hostUrl + "/url_mgmt/excel_url";
    // console.log(hostUrl, '*************', this.excel_url);
  },
  methods: {
    splitData(list) {
          ...
          if (res.status == 0) {
            ...
            for (var i = 0; i < nowarr.length; i++) {
              for (var j = 0; j < this.tableList.length; j++) {
                if (this.tableList[j].url_name == nowarr[i][0]) {
                  if (nowarr[i][1] === 1) {
        ...
        if (res.err_code == 0) {
          ...
          for (var i = 0; i < res.data.length; i++) {
            ...
            var respath = /^\/{1}[0-9a-zA-Z\/\+\?%#&=]{1,1024}$/; //路径
              ...
              res.data[i].err_buserid = false;
              this.disable = true;
              res.data[i].status += "渠道ID错误\n";
            }
            ...
            if (this.getBLen(res.data[i].domain) < 4) {
              res.data[i].err_url_name += "视频名称不能小于4字符\n";
              res.data[i].err_url_name = false;
            }
            if (this.getBLen(res.data[i].domain) > 1024) {
              res.data[i].err_url_name += "视频名称不能大于1024字符\n";
              res.data[i].err_url_name = false;
            }
            ...
            if (resaccelerate.test(res.data[i].url_name) === false) {
