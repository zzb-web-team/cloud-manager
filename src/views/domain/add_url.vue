...
<script>
import { add_url, getterminal, check_label } from '../../servers/api';
export default {
	data() {
		return {
			dynamicValidateForm: {
	...
	methods: {
		onchanidChange() {
			...
			if (
				this.dynamicValidateForm.newbuser_id !=
				this.dynamicValidateForm.bind_id
			) {
				this.queryInfoLabel();
			}
			return false;
		...
		queryInfoLabel() {
			...
			getterminal(param)
				.then((res) => {
					if (res.status == 0) this.labelData = [];
