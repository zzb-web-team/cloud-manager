    ...
    <div class="bath" style="width: 1240px;margin: auto;background: #ffffff;margin-top: 25px;border-radius: 2px;padding: 15px;box-shadow:0px 2px 3px 0px rgba(6,17,36,0.14);">
        <el-tabs :tab-position="tabPosition" v-model="oneName" @tab-click="handleClick">
            <div class="tala_title">
                <span style="width: 250px; float:left;overflow: hidden;text-overflow: ellipsis;white-space: nowrap;">{{ datalist.url }}</span>
                <div>
                    ...
                    <el-button @click="setquery_url()" style="background:#0ABF5B;color:#ffffff;width:76px;height:34px;line-height: 10px;">保存</el-button>
                </div>
            </div>
            ...
            <el-tab-pane label="基础配置" name="one">
                ...
                datalist.create_time | settimes
                    ...
                    <li>
                        <span>URL地址</span>
                        <span style="width: 250px; float:left;overflow: hidden;text-overflow: ellipsis;white-space: nowrap;" class="tala_con">{{ datalist.url }}</span>
                    </li>
                    <li>
                        <span>主标签</span>
                ...
                xas == false ? (datalist.label != "" ? "修改" : "设置") : "确定"
                    ...
                    <li>
                        ...
                        <el-select v-model="datalist.label2" placeholder="请选择" :disabled="!fas">
                            ...
                            <el-option v-for="item in options" :key="item.value" :label="item.label" :value="item.value" class="tala_con" style="width:300px;"></el-option>
                        </el-select>
                ...
                <div class="talb">
                    ...
                    <div v-show="urlno == true">
                        <div class="talb_title_two">
                            <span>回源URL地址</span>
                            <span>URL：{{ datalist.host_url.url }}</span>
                    ...
                    <el-tab-pane label="缓存过期时间" name="second">
                        ...
                        <div class="batch_huan">
                            <div style="text-align:left;padding: 10px 0">
                                <el-button type="primary" style="width:100px;" @click="toAddTab"><span class="el-icon-plus"></span>配置</el-button>
                            </div>
                            <!-- 缓存添加弹窗 -->
                            <el-dialog title="配置缓存过期时间" :visible.sync="huanVisible">
                                <el-form :model="huanform">
                                    <el-form-item label="过期类型:" style="text-align:left;" :label-width="formLabelWidth">
                                        <el-radio-group v-model="radioType" @change="onchangeTab">
                                            <el-radio-button label="自动过期"></el-radio-button>
                                            <el-radio-button label="自定义过期时间"></el-radio-button>

                                        </el-radio-group>
                                        <!-- <el-button :autofocus="true" type="primary" plain @click="automatic()">自动过期</el-button>
                                    ...
                                    <el-form-item label="区域:" style="text-align:left;" :label-width="formLabelWidth" class="huancunoption">
                                        <el-cascader :disabled="cityType" :options="citylist1" ref="cascaderAddr" v-model="citylabel"></el-cascader>
                                    </el-form-item>
                            ...
                            <el-table :data="datalist.cache_con" border :cell-style="rowClass" :header-cell-style="headClass">
                                <el-table-column prop="area" label="区域"> </el-table-column>
                                <el-table-column prop="province" label="城市">
                                </el-table-column>
                                <el-table-column prop="expire" label="过期时间">
                                    <template slot-scope="scope">
                                        <span v-if="scope.row.expire == 0">自动过期</span>
                        ...
                        scope.row.state == 0 ? "缓存中" : "已过期"
                                ...
                                <el-table-column label="操作">
                                    <template slot-scope="scope">
                                        ...
                                        <!-- <el-button type="text" size="small" style="color:#000000" @click="delhc(scope.row, scope.$index)">删除</el-button> -->
                                    </template>
                                </el-table-column>
                            </el-table>
                        </div>
                ...
                <div style="text-align: left;margin: 15px 0;" class="customize">
                    ...
                    <el-dialog title="自定义错误码" :visible.sync="dialogFormVisible" @close="handleClick4">
                        <el-form :model="form" ref="form">
                            <el-form-item label="错误码" prop="errCode" :label-width="formLabelWidth" :rules="[{ validator: jiourl1, trigger: 'blur' }]">
                                <el-input v-model.number="form.errCode" placeholder="请输入错误码，如（105，403）" autocomplete="off"></el-input>
                            </el-form-item>
                    ...
                    <el-table :data="datalist.custom_page" stripe style="width: 100%" :cell-style="rowClass" :header-cell-style="headClass">
                        ...
                        <el-table-column label="操作">
                            <template slot-scope="scope">
                                ...
                                <el-button type="text" size="small" style="color:#000000" @click="delzi">删除</el-button>
                            </template>
                        </el-table-column>
                    </el-table>
                </div>
...
export default {
    ...
    mounted() {
        ...
        this.citylabel = ["西南", "贵州"];
        // if (this.$cookies.get("id")) {
        //   this.chanid = this.$cookies.get("id") * 1;
        // } else {
        //   this.$router.push({ path: "/" });
        // }
        console.log(this.$route.query.urlress, this.$route.query.tabpne);
    ...
    methods: {
        ...
        geturlconfig() {
            ...
            query_url(parmas)
                .then(res => {
                    ...
                    // concash.expire_time = res.data.result[0].cache_config.expire_time;
                    // concash.weight = res.data.result[0].cache_config.weight;
                    // concash.url = res.data.result[0].url;
                    // this.datalist.cache_con.push(concash);
        ...
        //修改url
        setquery_url() {
            console.log(this.datalist.label, this.oldlabel, "/*/*/*/***");
        ...
        zidingyi(dats) {
            ...
            console.log(this.datalist.custom_page[dats].state);
        },
        //删除自定义状态
        delzi() {
            this.datalist.custom_page = [];
        },
        changeSwitch() {
            console.log(this.valuek);
            console.log(this.urlno);
        ...
        huanVisib() {
            ...
            if (this.edit == 1) {
                // let data = new Object();
                // data.type = this.huanform["type"];
                // data.area = this.huanform["area"];
                // data.province = this.huanform["province"];
                // data.expire = this.huanform["expire"];
                // data.state = this.huanform["state"];
            ...
            this.huanfo = "";
            this.automatic_time = true;
            console.log(this.datalist.cache_con, "hasgdhjgsda");
        },
        //添加缓存--取消
        huanno() {
            this.huanVisible = false;
        ...
        delhc(data, num) {
            ...
            // this.datalist.cache_con = [{ expire_time: 0, valid: 0, weight: 0 }];
    ...
    beforeRouteLeave(to, form, next) {
        if (this.setstate == true) {
            this.$confirm("修改的内容未保存，确定离开吗？", "提示", {
                    confirmButtonText: "确定",
                    cancelButtonText: "取消",
                    type: "warning"
                })
