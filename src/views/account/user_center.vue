	...
	<div class="content">
		...
		<ol>
			...
			<!-- <li>
				...
				<span class="item_con">{{ userinf.nickname ? userinf.nickname : '未设置' }}
				</span>
				<span class="item_btn">
					<el-button type="text" size="small" @click="setnickname">{{ userinf.nickname ? '修改' : '设置' }}
					</el-button>
				</span>
			</li> -->
			<li>
				<span class="item_title">真实姓名：</span>
				<span class="item_con">{{ userinf.name ? userinf.name : '未设置' }}
				</span>
				<span class="item_btn">
					<el-button type="text" size="small" @click="setname">{{ userinf.name ? '修改' : '设置' }}
					</el-button>
				</span>
			</li>
			...
			<li>
				...
				<span class="item_con">{{ userinf.phone ? userinf.phone : '未设置' }}
				</span>
				<span class="item_btn">
					<el-button type="text" size="small" @click="setphone">{{ userinf.phone ? '修改' : '设置' }}
					</el-button>
				</span>
			</li>
			<li>
				<span class="item_title">安全级别：</span>
				<span class="item_con">{{ grade }}
			...
			<li>
				...
				<span class="item_con">{{ userinf.google == 0 ? '未启用' : '已启用' }}
				</span>
				<span class="item_btn">
					<el-button type="text" size="small" @click="setpwd(1)">设置</el-button>
				</span>
			</li>
		</ol>
		</div>
		<!-- 弹窗 -->
		<el-dialog :title="tabtitle" :visible.sync="dialogFormVisible" customClass="customWidths" @close="handleClose2">
			<el-form :model="form" ref="ruleForm">
				<el-form-item prop="name" :rules="[{ validator: jiophone, trigger: 'blur' }]">
					<el-input v-model="form.name" autocomplete="off" :placeholder="placeholdercon"></el-input>
				</el-form-item>
			</el-form>
			<div slot="footer" class="dialog-footer">
				<el-button @click="nohandleSubmit('ruleForm')">取 消</el-button>
				<el-button type="primary" @click.native.prevent="handleSubmit4">确 定</el-button>
			</div>
		</el-dialog>
		<el-dialog title="验证" :visible.sync="recodecisbity" customClass="recustomWidth" @close="handleClose">
			<el-form :model="reform" ref="ruleFormre">
				<el-form-item class="reimg">
					<img :src="reform.url" alt="" />
					<div>
						<span>secret:</span>
						<span>{{ reform.secret_num }}</span>
					</div>
				</el-form-item>
				<el-form-item prop="name" :rules="[{ validator: jioyzm, trigger: 'blur' }]">
					<el-input v-model.number="reform.name" autocomplete="off" placeholder="验证码" maxlength="6"></el-input>
				</el-form-item>
			</el-form>
			<div slot="footer" class="dialog-footer">
...
export default {
  ...
  this.userinf=this.$cookies.get('clouduser')
  console.log(this.userinf)
    this.ipfs_id = parseInt(this.$cookies.get("adminid"));
    this.ipfs_user = this.$cookies.get("adminuser");
    if (this.userinf.google == 1 && this.userinf.phone != 0) {
      this.grade = "高";
    } else if (this.userinf.google == 1 && this.userinf.phone == 0) {
      this.grade = "中";
    } else {
      this.grade = "低";
    }
        ...
        .then(res => {
          ...
          if (res.status == 0) {
			...
			console.log(JSON.stringify(this.userinf))
			              this.$cookies.set("clouduser", JSON.stringify(this.userinf), 7 * 24 * 60 * 60);
