    ...
    <div class="user-title"
      ...
      <div class="resources_con">
        <div style="background:rgba(255,255,255,1);box-shadow: 0px 0px 6px 0px rgba(51, 51, 51, 0.16);border-radius:2px;padding:30px 25px;">
          <div class="top_title">渠道流量用量</div>
          <div style="display: flex;justify-content: space-between;align-items: center;flex-flow: row;">
            ...
            <el-button @click="showzdyx">
              ...
              <el-date-picker v-show="showzdy"
                              style="width: 295px;margin-left:10px;margin-top: 20px;"
                              v-model="val2"
                              type="date"
                              placeholder="选择日期"
                              align="left"
        ...
        <div class="device_form"
          ...
          <el-button type="primary"
                     icon="el-icon-plus"
                     id="sellineName"
                     @click="showduibi">添加对比</el-button>
        </div>
        ...
        <div class="devide_table">
          <el-row type="flex"
                  class="row_active">
            <el-col :span="24"
                    style="text-align:left;font-weight: bold;padding-left: 10px;">资源用量表</el-col>
          </el-row>
          <el-row type="flex"
                  class="row_active">
            <el-col :span="24">
              <el-table :data="tablecdn"
                        border
              ...
              <fenye style="float:right;margin:10px 0 0 0;"
                     @handleCurrentChange="handleCurrentChange"
                     @handleSizeChange="handleSizeChange"
                     :pagesa="total_cnt"
                     :currentPage="pageNo"></fenye>
            </el-col>
          </el-row>
        </div>
      </div>
  ...
  methods: {
    ...
    timeFormatter(row, column) {
      if (this.timeUnit == 120) {
        ...
        return (
          this.common.getTimes(startTime) + '-' + this.common.getTimes(endTime)
        )
      } else {
    ...
    //自定义时间
    gettimes(cal) {
      this.starttime = parseInt(this.val2 / 1000)
      this.endtime = parseInt(this.val2 / 1000) + 86399
      this.$refs.multipleTable.clearSelection()
      this.pageNo = 1
      this.gettable1()
      this.gettable2()
    ...
    drawLine(x, y, idlist) {
              ...
              if (params[i].seriesName.includes('series')) {
                str +=
                  ...
                  common.formatByteActive(
                    Number(
                      _this.curveData[0].dataflowArray[params[i].dataIndex]
                    )
                  ) // + _this.flowunit;
              } else {
      ...
      //     let happynewmonth = Number(shold_time.split('-')[0])
      //     let happynewday = Number(shold_time.split('-')[1])
      //     let StartTime = happynewyear + '-' + happynewmonth + '-' + happynewday
      //     _this.showzdy = true
      //     console.log(params)
      //     _this.val2 = Number(new Date(StartTime))
